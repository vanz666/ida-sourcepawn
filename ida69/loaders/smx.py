...
def process_imports(natives, sel, start, end, name, sclass):
    ...
    for native in natives:
        idc.MakeDword(ea)
        idaapi.do_name_anyway(ea, native)
    ...
    if code_ea != idaapi.BADADDR:
        for public in image.publics:
            ...
            if entry_ea >= (code_ea + image.code.size):
                continue
            idaapi.add_entry(entry_ea, entry_ea, public.name, 1)
    ...
    if data_ea != idaapi.BADADDR:
        for pubvar in image.pubvars:
            ...
            if entry_ea >= (data_ea + image.data.size):
                continue
            idaapi.add_entry(entry_ea, entry_ea, pubvar.name, 0)
...
def load_file(li, neflags, format):
    idaapi.set_processor_type("srcpawn", idaapi.SETPROC_ALL|idaapi.SETPROC_FATAL)
    ...
    idc.SetLongPrm(idc.INF_MAX_EA, 0x10000)
    ...
    idc.SetShortPrm(idc.INF_START_AF, GetShortPrm(idc.INF_START_AF) & ~idc.AF_IMMOFF)

    idc.set_start_cs(cs_sel)
